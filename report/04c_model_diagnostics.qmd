...
Forcing asymptotic selectivity on the midwater fleet increased estimated spawning biomass in 2025 by `r sens_base_ssb[which(mod_names=="forcing asymptotic selectivity on the midwater trawl fleet")]-100`% compared to base model, while forcing logistic selectivity on the NWFSC WCGBT resulted in similar estimated spawning biomass to the base model. Including shrimp trawl data decreased estimated spawning biomass `r sens_base_ssb[which(mod_names=="including previously excluded shrimp trawls")]`% and decreased recruitment deviations in the 2019-2025 period [@fig-recdevs-sens]. Updating WA catch reconstruction and excluding the triennial survey data had almost no impact on the estimated spawning biomass. 

The alternative weighting using the Francis method generally increased the estimate of spawning biomass across the timeseries. While the spawning biomass from the Francis weighted model is `r sens_base_ssb[which(mod_names=="weighting the composition data using the Francis method")]`% higher than that from the base model, the OFL coming out of that model is actually lower than the base model.
