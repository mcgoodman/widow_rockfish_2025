...
The time series of estimated \gls{ssb} and relative \gls{ssb} is in @fig-b and @fig-c, respectively, and for the most recent year in @tbl-b. The \gls{ssb} declined rapidly with the developing domestic midwater fishery in the late 1970s and early 1980s. Low abundance continued until 2000. A combination of strong recruitment and low catches resulted in a steady increase in \gls{ssb} through 2016. A target fishery for widow rockfish was reestablished in 2017. The stock exhibits a decline in most recent years with the increased catches since 2017.

The 2025 \gls{ssb} relative to unfished equilibrium \gls{ssb} is `r round(table_b$Bratio[table_b$Year == 2025] * 100)`, above the target of 40% (95% confidence interval of `r round(table_b$Bratio_Lwr[table_b$Year == 2025] * 100)` - `r round(table_b$Bratio_Upr[table_b$Year == 2025] * 100)`%). 
...
#| warning: false
#| eval: true
#| fig-cap: "Estimated female \\gls{ssb} time-series from the base model (solid line) with an approximate asymptotic 95% confidence interval (dashed lines)."
#| out.width: "90%"
#| fig-align: "center"
...
#| warning: false
#| eval: true
#| fig-cap: "Estimated relative \\gls{ssb} (depletion) with approximate 95% asymptotic confidence intervals (dashed lines) for the base case assessment model."
#| out.width: "90%"
#| fig-align: "center"
...
flextable(table_B) |> 
  colformat_num(j = 1, big.mark = "", digits = 0)%>%
  # mutate(
  #   "Fraction Unfished" = format("Fraction Unfished",3)
  # )
  set_table_properties(layout = "autofit") %>% exec_tab_style %>% 
  width(width = 0.6)
...
#| echo: false
#| warning: false
#| tbl-cap: "Recent trend in \\gls{spr} and summary exploitation rate. Harvest rate is defined as catch divided by age 4+ biomass."
...
flextable(table_d) %>%
    colformat_num(j = 1, big.mark = "") %>%
  exec_tab_style %>% width(width = 0.85)
