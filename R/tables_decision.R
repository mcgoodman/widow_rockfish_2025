# Author: Mico Kineen
# Code to implement a decision table for Widow rockfish for the 2025 assessment
...
if (FALSE) {
  # skipping this section as fleet grouping is changed and base model has GMT catches in it already
  # GMT data
  gmt_catch <- read.csv(here(
    "data_provided",
    "GMT_forecast_catch",
    "GMT_forecast_catch.csv"
  )) |>
    mutate(seas = 1) |>
...
run_CC <- FALSE # turn off CC scenario temporarily
run_40 <- FALSE # turn off p*0.40 scenario temporarily
if (run_CC) {
  # turn off CC scenario temporarily
...
if (run_40) {
  # Temporarily turn off p*0.40 scenarios
  ...
  p_star_40$fore$Flimitfraction_m <- PEPtools::get_buffer(
    2025:2036,
    sigma = 0.5,
    pstar = p_star
  )
...
p_star_45$fore$Flimitfraction_m <- PEPtools::get_buffer(
  2025:2036,
  sigma = 0.5,
  pstar = p_star
)
...
# p_star_45$fore$ForeCatch <- gmt_catch # already included in base model
...
catch_series_list <- parLapply(cl, dirs, function(x) {
  ...
  ### these commands now dealt with later in the script
...
pars_table <- rep$parameters |> # make sure to ppull from report, not input file so est values are used
  mutate(
    value = Value,
    sd = if_else(is.na(Parm_StDev), Pr_SD, Parm_StDev)
  ) |> # If est par, use param sd, otherwise use prior sd
  ...
  mutate(
    ...
    phase = -1,
    name = c(
      "NatM_p_1_Fem_GP_1", # name the parms - easier to do manually as report and r4ss::read() have different parameter names
      "NatM_p_1_Mal_GP_1",
      "SR_BH_steep"
    )
  )
...
# Dirs
if (run_CC & run_40) {
  # temporarily skipping CC and p*0.40 scenarios
  dirs <- c(
    here("data_derived", "decision_table", "cc_low"), # low SON, P*0.40
  ...
  # temporarily creating a smaller set of dirs for low and high states of nature only
  dirs <- c(
    here("data_derived", "decision_table", "45_low"), # low SON 45
    here("data_derived", "decision_table", "45_high") # High SON 45
  )
}
...
for (i in seq_along(dirs)) {
  ...
  # Copy the base model
  copy_SS_inputs(
    dir.old = base_mod_dir, # cc_base_dir,
    dir.new = temp_dir,
    overwrite = TRUE,
    create.dir = TRUE,
    copy_par = TRUE,
  ...
  SS_changepars(
    dir = temp_dir,
    ctlfile = "2025widow.ctl",
    newctlfile = "2025widow.ctl",
    strings = pars_table$name,
    newvals = new_vals,
    newphs = pars_table$phase,
    verbose = FALSE
  ) # seeting negative, so not estimate
  ...
  SS_write(inputlist = mod, dir = temp_dir, overwrite = TRUE)
...
if (run_CC & run_40) {
  all_dirs <- c(
    here("data_derived", "decision_table", "cc_low"),
...
result <- tryCatch(
  {
    ...
    testthat::expect_equal(
      object = SS_decision_table_stuff(
        dec_table_reps[["45_base"]],
        yrs = 2025:2036,
        digits = c(0, 5, 3)
      )$SpawnBio,
      expected = projections$table$`Spawning output (billions of eggs)`,
      tolerance = 0.00001
    )
...
if (grepl("PASS", result)) {
  ...
  write.csv(
    dec_table_results,
    file = here("data_derived", "decision_table", "dec_table_results.csv"),
    row.names = FALSE
  )
  ...
  if (run_CC) {
    compare_ss3_mods(
      replist = list(
        dec_table_reps[["cc_base"]],
        dec_table_reps[["cc_low"]],
        dec_table_reps[["cc_high"]]
      ),
  ...
  compare_ss3_mods(
    replist = list(
      dec_table_reps[["45_base"]],
      dec_table_reps[["45_low"]],
      dec_table_reps[["45_high"]]
    ),
  ...
  if (run_40) {
    compare_ss3_mods(
      replist = list(
        dec_table_reps[["40_base"]],
        dec_table_reps[["40_low"]],
        dec_table_reps[["40_high"]]
      ),
  ...
  if (run_CC & run_40) {
    ...
    pnl_40 <- pnl_40 |>
      image_annotate("ACL = p*0.40", size = 48, location = "+200+0")

    pnl_45 <- image_read(here(
      "figures",
      "decision_table",
      "45_plots",
      "compare4_Bratio_uncertainty.png"
    ...
    pnl_45 <- pnl_45 |>
      image_annotate("ACL = p*0.45", size = 48, location = "+200+0")
    ...
    image_write(
      combined,
      path = here("figures", "decision_table", "combined_ssb_with_interval.png")
    )
  } # end if(run_CC & run_40)
  ...
  write.csv(
    dec_table_formatted,
    here("report", "tables", "dec_table_formatted.csv"),
    row.names = FALSE
  )
  write.csv(
    dec_table_formatted,
