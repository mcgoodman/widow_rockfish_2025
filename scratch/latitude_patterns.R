...
# Get data
observer <- read.csv(
  'data_provided/wcgop/ward_oken_observer_data_2024-09-10.csv'
) |>
  as_tibble() |>
  filter(
...
em <- read.csv('data_provided/wcgop/ward_oken_em_data_2024-12-03.csv') |>
  as_tibble() |>
  filter(species == 'Widow Rockfish', sector == 'Midwater Rockfish EM') |>
  mutate(DRVID = as.character(DRVID))

widow_catch <- nwfscSurvey::pull_catch(
  survey = 'NWFSC.Combo',
  common_name = 'widow rockfish'
)
widow_bio <- nwfscSurvey::pull_bio(
  survey = 'NWFSC.Combo',
  common_name = 'widow rockfish'
)
...
catch_by_lat <- bind_rows(observer, em) |>
  ...
  labs(
    x = 'Total midwater rockfish catch, 2012-2023 (mt)',
...
bind_rows(observer, em) |>
  ...
  filter(is_ocean) |>
  left_join(
    y = ggplot_build(catch_by_lat)$data[[1]],
...
widow_map <- widow_catch |>
  ...
  ggplot() +
  geom_point(
    aes(x = Longitude_dd, y = Latitude_dd, size = cpue_kg_km2, col = yr_bin),
    alpha = 0.25
  ) +
  guides(
    color = guide_legend(
      override.aes = list(alpha = 1, linetype = 1),
      title = 'Year bin'
    )
  ) +
...
### figure for SSC review

load('data_provided/WCGBTS/delta_lognormal/index/predictions.Rdata')
library(data.table)

binned_lat <- setDT(predictions)[,
  lat_bin := cut(
    latitude,
    breaks = na.omit(ggplot2::ggplot_build(catch_by_lat)$data[[1]]$ymin)
  )
] |>
  mutate(
    pred = boot::inv.logit(est1) * exp(est2),
    min_lat = as.numeric(stringr::str_extract(
      lat_bin,
      '([:digit:]|\\.)+(?=,)'
    )),
    max_lat = as.numeric(stringr::str_extract(
      lat_bin,
      '([:digit:]|\\.)+(?=\\])'
    )),
  ...
  group_by(mean_lat) |>
  summarise(summed_bio = sum(pred))

survey_bio_by_lat <- binned_lat |>
  ggplot() +
  geom_col(aes(x = mean_lat, y = summed_bio)) + # KLO has checked weight argument works as expected, plots total MT/bin
  theme(legend.position = 'none') +
  coord_flip() +
  scale_y_reverse() +
  scale_x_continuous(limits = c(32.5, 49), expand = c(0, 0)) +
  labs(
    y = 'Widow biomass in WCGBTS (summed over yrs)',
