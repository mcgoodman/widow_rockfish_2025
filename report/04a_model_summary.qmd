...
### Changes from the Last Assessment and Model Bridging {#sec-bridging}
...
- **Streamlining the fleet structure** (as described under "Data", @sec-data). The landings from fleets with minimal and mostly historical catches (hook-and-line and net) were added to the bottom trawl landings with hook-and-line and net composition data removed from the model.
- **Use external estimates of discards rather than estimate within the model**. Methods, used to reconstruct discards in the bottom trawl and midwater trawl fleets, based on the Pikitch, EDCP and WCGOP data are described under "Data" (@sec-data). All retention parameters were removed from the model.
...
Fixed parameters in the model included steepness, standard deviation of recruitment deviates, maturity at age, fecundity-at-length, and length-weight parameters. Consistent with the previous assessment [@adams_status_2019], steepness was fixed at 0.72, matching the mean of the current west coast rockfish steepness prior [@thorson_steep_2019], described below under "Priors" (@sec-priors). A likelihood profile was conducted for steepness. The standard deviation of recruitment deviates was fixed at 0.60. Maturity at age was fixed as described in the previous update assessment, @adams_status_2019. Fecundity-at-length parameters were fixed at values from @dick_meta-analysis_2017. Length-weight parameters were fixed at estimates using length-weight observations from the \gls{s-wcgbt} (@fig-wl-fits and @tbl-params). 
...
#### Recruitment deviations {#sec-recruitment-deviations}
