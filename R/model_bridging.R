...
#  --------------- Add the new forecast catches
gmt_forecast <- read.csv(here("data_provided","GMT_forecast_catch","GMT_forecast_catch.csv"))

#data
forc <- r4ss::SS_readforecast(here("models", "2025 base model","forecast.ss"))

forc$ForeCatch <- gmt_forecast|>select(-c(fleet_name,Source)) #overwrite the catches
r4ss::SS_writeforecast(mylist = forc,dir = here("models", "2025 base model"),overwrite = T)
