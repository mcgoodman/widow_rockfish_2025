...
A number of tests were performed to verify convergence of the model. Following conventional AD Model Builder methods [@fournier_ad_2012], we checked that the Hessian matrix for the base model was positive-definite. There were no difficulties in inverting the Hessian to obtain asymptotic variance estimates.
...
Sensitivity analysis was performed to determine the model behavior under different assumptions than those of the base case model. Broad range of sensitivity runs were conducted at multiple stages of the assessment. Here, we present runs related to different assumptions about natural mortality, because the model and derived management quantities exhibit sensitivity to this parameter. The sensitivity runs include the following:
...
Likelihood values and estimates of key parameters are shown in @tbl-quant-sens-int. Predicted trajectories of spawning output, fraction unfished, recruitments and recruitment deviations comparisons of model estimates for 2025 are shown in @fig-spawnbio-sens-uncrt,  @fig-depl-sens, @fig-rec-sens, and @fig-recdevs-sens.  
