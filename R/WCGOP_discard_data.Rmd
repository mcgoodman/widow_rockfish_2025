...
ssdat.2019$lencomp %>% filter(part==1) %>% filter(year>2004) %>% 
  mutate(fleet=as.factor(as.character(fleet))) %>% 
  ggplot(aes(x=fleet, y=Nsamp)) + 
...
```{r}
# Code not working fully yet - figure out why 1980's not plotting (likely b/c it's by male & female)
...
# dev.new(width=5, height=20, unit="in")
# {plot(length.comps.agg$year, length.comps.agg$variable, xlab = "Year", ylab = "Length (cm)", main = "Discards by length by year",
#      xlim = range(length.comps.agg$year), ylim = c(8, 62), type = "n")
# symbols(length.comps.agg$year, length.comps.agg$variable, circles = sqrt(length.comps.agg$value/pi), inches = 0.1, add = TRUE, fg = c("red","blue")[length.comps.agg$sex], bg = "white")
# 
# 
# # Legend
# legend_x <- 1985
...
# legend_circles <- sqrt(legend_sizes/pi)
# symbols(legend_x-0.5 + (1:length(legend_sizes)) * 3,rep(legend_y+0.5, length(legend_sizes)) ,
#         circles = legend_circles, inches = 0.1, add = TRUE, bg = "white", fg=length.comps.agg$sex)
# text(legend_x + (1:length(legend_sizes)) * 3,rep(legend_y + 1, length(legend_sizes)) ,
#      labels = legend_sizes, adj = 0)}
...
length.comps.agg %>% mutate(sex=as.factor(as.character(sex))) %>% 
  filter(value>0) %>% 
  ggplot(aes(x = year, y = variable, col = as.factor(sex), size = value)) +
                   ggplot2::geom_point(shape=1, position=position_dodge(0.5)) + 
  scale_color_manual(values=c("red","blue", "purple")) + theme_cleveland() + 
  ylim(c(10,60)) + 
  ylab("Length (cm)") + xlab("Year") + ggtitle("Discard length counts, all fleets\nIncludes midwater hake/rockfish")


# NO MIDWATER 

length.comps.agg.nomid <- length.comps %>%
...
# dev.new(width=5, height=20, unit="in")
# {plot(length.comps.agg.nomid$year, length.comps.agg.nomid$variable, xlab = "Year", ylab = "Length (cm)", main = "Discards by length by year",
#      xlim = range(length.comps.agg.nomid$year), ylim = c(8, 62), type = "n")
# symbols(length.comps.agg.nomid$year, length.comps.agg.nomid$variable, circles = sqrt(length.comps.agg.nomid$value/pi), inches = 0.1, add = TRUE, fg = c("red","blue")[length.comps.agg.nomid$sex], bg = "white")
# 
# 
# # Legend
# legend_x <- 1985
...
# legend_circles <- sqrt(legend_sizes/pi)
# symbols(legend_x-0.5 + (1:length(legend_sizes)) * 3,rep(legend_y+0.5, length(legend_sizes)) ,
#         circles = legend_circles, inches = 0.1, add = TRUE, bg = "white", fg=length.comps.agg.nomid$sex)
# text(legend_x + (1:length(legend_sizes)) * 3,rep(legend_y + 1, length(legend_sizes)) ,
#      labels = legend_sizes, adj = 0)}

length.comps.agg.nomid %>% mutate(sex=as.factor(as.character(sex))) %>% 
  filter(value>0) %>% 
  ggplot(aes(x = year, y = variable, col = as.factor(sex), size = value)) +
                   ggplot2::geom_point(shape=1, position=position_dodge(0.5)) + 
  scale_color_manual(values=c("red","blue", "purple")) + theme_cleveland() + 
  ylim(c(10,60)) + 
  ylab("Length (cm)") + xlab("Year") + ggtitle("Discard length counts, all fleets\nBottom trawl and Hook & Line only")
