...
Reference points were calculated using the estimated selectivity parameters and average catch distribution among fleets in the most recent five years of the model (2019-2024). A list of estimates of the current state of the population, as well as reference points based on 1) a target unfished spawning output of 40%, 2) a \gls{spr} of 0.5, and 3) the model estimate of maximum sustainable yield (MSY), are all listed in @tbl-e ADD TABLE TO THE MAIN DOCUMENT. The \gls{spr} is expected lifetime reproductive output under a given fishing intensity as a proportion of the unfished expected lifetime reproductive output. 
...
While the estimate of population scale has remained relatively steady across the 2015, 2019, and 2025 assessments, natural mortality (M) have declined from 2015 (female M of 0.157 yr^-1^) to 2019 (0.144 yr^-1^) to 2025 (`r round(report$M_at_age[1, 4], 3)` yr^-1^), reflecting the increase of the number of older fish in the population throughout rebuilding years. Thus, although the population scale is estimated consistently, additional years of data (in particular, age composition data) following the re-opening of the fishery have led the model to estimate that the population is less productive, which results in lower yields at all three reference points (proxy based on spawning output, proxy based on fishing intensity, and model estimate of MSY). Also, the model did not support  large 2013 year class previously estimated in the 2019 assessment, which contributed to decrease in of stock productivity and estimated OFLs.

The equilibrium yield plot is shown in @fig-est-eqi-yield, based on a steepness value fixed at 0.720. The predicted maximum sustainable yield under the assumptions of this assessment occurs near 27% of equilibrium unfished spawniong output, however this represents only 12% higher yield with considerably more risk than under the 50% SPR policy, which occurs near 45% of unfished spawning output. 
...
first two years of the projection period. The predicted OFL is the calculated total catch determined by FSPR=50% are provided in TABLE (TO DO: ADD THE REFERENCE TO THE TABLE WITH HARVEST PROJECTIONS).
...
The decision table with projections under alternative catch streams is to be provided after the review meeting.
