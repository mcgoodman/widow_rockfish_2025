...
# Add a row that spans all columns (like a section header)
 flextable(table34) %>%
 set_header_labels(values = c("Fleet",
               "Composition data type", 
               "Log Mean effN  2025 base model" = "Log(Mean effN): 2025 base model" , 
               "Log Mean effN  Francis" = "Log(Mean effN): Francis",
               "Base model McAllister Ianelli weighting" = "McAllister Ianelli weighting (Base model)",
               "Francis weighting" = "Francis weighting (Alternative)")) %>%
 style_table34() %>% 
...
flextable(table35) %>% 
 ...
 bold(part = "header")%>% border(border.top = my_border, i = 1) %>% 
 autofit() %>% 
 colformat_num(j = ncol(table35), big.mark = "", digits = 0)%>% 
 fontsize(size = 9, part = "all")%>% 
...
table36$scenario <- plyr::revalue(table_h$scenario,
