...
#  width(width = 0.9, j = 1)
table36 <- table36 %>% filter( scenario != "Constant catch (9,000 K)") %>% select(2:ncol(table36)) # remove X col
flextable(table36) |>
 set_header_labels(values = c(scenario = "Management decision",
               yr = "Year", 
               # OFL = "OFL", 
 ...
 colformat_num(j = 2, big.mark = "", digits = 0) |>
 border_inner(part="header") |> border_outer() |> # header and outside border
 vline(j = c(1, 3, 5, 7)) |> hline(i = 12) |> hline(i = 24) |>
 merge_at(i = c(1:12), j = 1) |> merge_at(i = c(13:24), j = 1) |>
 # set_table_properties(layout = "autofit", align = "center") |> 
 width(width = 0.6, j = 2:(dim(table36)[2]-1)) |> 
 width(width = 0.9, j = 1)
