...
#| warning: false
#| eval: true
#| fig-cap: "Estimated fraction of unfished spawning output with asymptotic 95% confidence interval (dashed lines) for the base model."
#| out.width: "90%"
#| fig-align: "center"
...
dec_table_res <- read.csv(here("data_derived", "decision_table", "dec_table_with_OFL_formatted.csv"))
...
A twelve-year projection of the base model with one catch stream based on \gls{acl} = \gls{abc} based on the adjustment of P&#42; = 0.45 was conducted (@tbl-h). The projections indicate that the spawning output is likely to initially decrease under all states of nature before partially rebounding  (less so under the low state of nature). Predicted \gls{acl} catches range from `r format(ctch_rng$catch[ctch_rng$stat == "min" & ctch_rng$scenario == "P*0.45"], big.mark = ",", digits = 1)` mt in `r ctch_rng$yr[ctch_rng$stat == "min" & ctch_rng$scenario == "P*0.45"]` to `r format(ctch_rng$catch[ctch_rng$stat == "max" & ctch_rng$scenario == "P*0.45"], big.mark = ",", digits = 1)` mt in `r  ctch_rng$yr[ctch_rng$stat == "max" & ctch_rng$scenario == "P*0.45"]`. 
